...
public class EvaluateActionMonthHandler
{
    ...
    public EvaluateActionMonthHandler(
        ZenTamDbContext         db,
    ...
    public async Task<EvaluateActionMonthResponse> HandleAsync(
        ...
        CancellationToken ct = default)
    {
        ...
        // 3. Convert target solar date (year/month/day) to lunar
        var targetSolarDate = new DateTime(request.TargetYear, request.TargetMonth, request.TargetDay);
        var targetLunarContext = _lunar.Convert(targetSolarDate);

        // 4. Get CanChi for year and month
        ...
        // 7. Build LunarDateContext with Year + Month
        var lunarContext = new LunarDateContext
        {
            ...
            Jdn        = targetLunarContext.Jdn,
            SolarMonth = targetSolarDate.Month
        };

        // 8. Resolve Year + Month tier rules
        ...
        var resolved = resolvedYear.Concat(resolvedMonth).ToList();
        ...
        // 12. Build LunarDateStr like "14/3 Bính Ngọ"
        string lunarDateStr = $"{targetLunarContext.LunarDay}/{targetLunarContext.LunarMonth} {canChiNam.Can} {canChiNam.Chi}";

        // 13. Build base response
        var response = new EvaluateActionMonthResponse
        {
            IsAllowed    = isAllowed,
        ...
        // 14. Fire-and-forget Gánh Mệnh if CAM (silent fail - bonus check)
        if (verdict == "CAM")
        {
            _ = Task.Run(async () =>
            {
